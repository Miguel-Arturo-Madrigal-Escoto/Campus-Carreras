...
$datosAlumnoQuery = "SELECT * FROM alumnos WHERE codigoAlumno = '$codigoAlumno'";
$datosAlumno = mysqli_query($conexion, $datosAlumnoQuery);
$datos = mysqli_fetch_assoc($datosAlumno);
...
function Footer()
{
     ...
     $this->Cell(0,10,'Los Belenes, Zapopan, Jalisco a ' . $emision . '.',0,0,'C');
     $this->Ln(25);
     $this->SetFont('Arial','B',10);
...
$dirigidoA = utf8_decode(mb_strtoupper($_POST['dirigido'])); 
$cargoDirEmp = utf8_decode(mb_strtoupper($_POST['cargo']));
$nombreEmp = utf8_decode(mb_strtoupper($_POST['nombreEmpresa'])); 
