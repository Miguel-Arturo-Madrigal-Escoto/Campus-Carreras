...
if(!isset($_SESSION['empresa'])){ header("Location: index.php"); }
    ...
    if(!preg_match('/[a-zA-Z]/', $nombre)){ header("Location: index.php"); }
    if(!preg_match('/[a-zA-Z0-9:,--_-]/', $direccion)){ header("Location: index.php"); }
    if(!preg_match('/[0-9---]/', $telefono)){ header("Location: index.php"); }
    ...
    $actualizarCuposQuery = "UPDATE cupos SET adminOriginal='$cupoAdmin', gestionOriginal='$cupoGestion', inforOriginal='$cupoInfor', telecOriginal='$cupoTelec', energOriginal='$cupoEnerg',electriOriginal='$cupoElectri',mecaOriginal='$cupoMeca' WHERE idEmpresa='$idEmpresa'";
    echo $actualizarCuposQuery;
    $actualizarCupos = mysqli_query($conexion, $actualizarCuposQuery);
    $ubicacion = "Location: index.php?em=" . md5(1);
    header($ubicacion);
